...
> **Project status:** `0.4.0-alpha.2` is technically ready for authorization and
> publication under npm's `next` dist-tag. Real consumer migrations intentionally
> follow that alpha release so they can lock one reproducible registry version.
...
Known source ancestry and the authorization required before the first public
release are recorded in [PROVENANCE.md](./PROVENANCE.md).
