...
- Release documentation now defines the alpha `next` dist-tag, immutable
  artifact verification, clean registry installation, and beta promotion gates.
...
- Explicit React wrapper sizing styles and a production example that loads the
  package CSS, so Cytoscape always receives a non-zero canvas height
...
- Exact-package fixtures cover Vue production bundling and static UMD delivery,
  including asset loading, controls, same-instance updates, and teardown.
- The release authorization record now identifies the approving party, review
  date, first-party scope, and reviewed third-party notices.
