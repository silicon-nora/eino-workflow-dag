...
var fixture = {
  version: 2,
  nodes: [
    { id: "a", kind: "io", name: "A", status: "success", cost_ms: 10, metrics: { segment: "reporter" } },
    {
      id: "g",
      kind: "graph",
      name: "G",
      ...
      graph: {
        nodes: [
          { id: "c1", kind: "cpu", name: "C1", status: "success", cost_ms: 20, metrics: { is_reporter: true } },
...
assert(aNode && aNode.metrics && aNode.metrics.segment === "reporter", "metrics passthrough on leaf");
...
assert(c1 && c1.metrics && c1.metrics.is_reporter === true, "nested metrics passthrough");
...
var hierRoot = {
  version: 2,
  nodes: [
    { id: "discover", kind: "io", cost_ms: 100, status: "success" },
    { id: "router", kind: "branch", cost_ms: 5, status: "success" },
    { id: "silent", kind: "cpu", cost_ms: 1, status: "skipped" },
    {
      ...
      graph: {
        nodes: [
          { id: "profile", kind: "io", cost_ms: 30, status: "success" },
          { id: "rank", kind: "cpu", cost_ms: 40, status: "success" },
          { id: "token", kind: "io", cost_ms: 10, status: "success" },
          { id: "sum", kind: "io", cost_ms: 80, status: "success" },
          { id: "body", kind: "io", cost_ms: 120, status: "success" },
          { id: "prep", kind: "merge", cost_ms: 5, status: "success" },
          { id: "work", kind: "llm", cost_ms: 200, status: "success" },
          { id: "write", kind: "io", cost_ms: 5, status: "success" }
        ],
        edges: [
  ...
  edges: [
    { from: "discover", to: "router" },
    { from: "router", to: "silent", kind: "branch" },
    { from: "router", to: "week_report", kind: "branch" }
  ]
};
var hierCol = Model.buildVisibleGraph(hierRoot, {});
assert(
  hierCol.criticalPath.join(",") === "discover,router,week_report",
  "collapsed root criticalPath Start→End (virtual)"
);
var hierExp = Model.buildVisibleGraph(hierRoot, { week_report: true });
assert(
  hierExp.criticalPath.join(",") === "discover,router,week_report",
  "expanded: root criticalPath still includes week_report wrapper"
);
assert(
  hierExp.edges.some(function (e) {
    return e.from === "router" && e.to === "week_report" && e.level === 1;
  }),
  "expanded: parent edge into Graph stays level 1"
);
assert(
  hierExp.edges.some(function (e) {
    return e.from === "week_report/rank" && e.to === "week_report/token" && e.level === 1;
  }),
  "expanded: inner longest branch rank→token is level 1"
);
assert(
  hierExp.edges.some(function (e) {
    return e.from === "week_report/token" && e.to === "week_report/body" && e.level === 1;
  }),
  "expanded: longer content arm token→body is level 1 (not sum)"
);
assert(
  !hierExp.edges.some(function (e) {
    return e.from === "week_report/token" && e.to === "week_report/sum" && e.level === 1;
  }),
  "expanded: shorter summary arm is level≥2"
);
assert(
  hierExp.edges.some(function (e) {
    return e.from === "week_report/work" && e.to === "week_report/write" && e.level === 1;
  }),
  "expanded: inner path reaches write before END"
);
...
// 扇出剥轨：load_summary + llm_work_items 为 level 2
var fanRoot = {
  version: 2,
  nodes: [
    {
      ...
      graph: {
        nodes: [
          ...
          { id: "profile", kind: "io", cost_ms: 100, status: "success" },
...
assert(fanNode("wg/token") && fanNode("wg/token").level === 1, "token level 1");
assert(fanNode("wg/prep") && fanNode("wg/prep").level === 1, "prep level 1");
...
assert(
  fan.edges.some(function (e) {
    return e.from === "wg/token" && e.to === "wg/topn" && e.level === 1;
  }),
  "token→topn is level 1 edge"
);
